...
public class TextLoader : MonoBehaviour {
  ...
  void Awake () {
	...
	void Update ()	{
		...
		if (activateOnPlay && !checkDone) {
			
			targetText.text = getText (filePath, originalText);
			checkDone = true;
		}
	}

  public static string getText (string path, string box, int index = 0) {
    ...
    if (jsonFinder != null) {
      var N = JSON.Parse (jsonFinder.ToString ());
			var textString = N[box][index].Value != null ? N[box][index].Value : path;
      return textString;
    }
    else
  ...
  public static string accessLang () {
		...
		return (GameManager.instance) ? GameManager.instance.lang.ToString () : "ENG";
