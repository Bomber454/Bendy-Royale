...
public class CustomInputModule : MonoBehaviour {
  ...
  public float GetAxis (string target, bool unclamped = false) {
      ...
      if (buttons[i].buttonName == target) {
        ...
        acquiredValue += (GetButton (target) ? 1 :
          (GetNegativeButton (target) ? -1 : 0));
			...
			//if(foundAxis) print(target + " " + buttons[i].buttonName + ": ACQ " + acquiredValue);
    ...
    //print (target + " acquires " + acquiredValue);
		...
		if(!unclamped)acquiredValue = Mathf.Clamp (acquiredValue, -1, 1);
    return acquiredValue;
  }

	public float GetAxisRaw (string target, bool unclamped = false) {
